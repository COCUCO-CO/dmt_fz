...
def PlotCorr(ax, dictio, band, cond, fdr=True, alpha=0.05, pvalue=0.05, ticks=False):
  ...
  #  corr_matrix[:, roi,score] = to_rgba(cm.plasma(r))
    ...
    if fdr==True:
      ...
      for roi, score in product(range(7), range(n_quest)):
...
ax.set_xlabel("Visual Network - Coherence")
...
#for band in band_list:
print(f"[INFO] Generating coherence histograms for conditions {cond_list[0]} vs {cond_list[1]} (hemi={hemi})")
...
for i, band in enumerate(band_list):
    for j, net in enumerate(net_list):
        ...
        cond1 = reject_outliers(np.asarray(cond1_flat, dtype=float)) if cond1_flat else np.array([])
        cond2 = reject_outliers(np.asarray(cond2_flat, dtype=float)) if cond2_flat else np.array([])
...
for band in band_list:
    print(f"[INFO] Generating network-pair histograms for band {band} ({cond_list[0]} vs {cond_list[1]}, hemi={hemi})")
