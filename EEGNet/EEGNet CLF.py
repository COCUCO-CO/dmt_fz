...
def func(cond):
    ...
    conc_data=data_list[0].get_data(copy=False)
    for i in range(1,len(data_list)):
        conc_data=np.concatenate((conc_data, data_list[i].get_data(copy=False)), axis=0)
    print(np.shape(conc_data))
    return conc_data

dmt_epochs = func("DMT")
closed_epochs = func("EC") # if fname not in ["S07_EC_ICA_pruned.set"]]
open_epochs = func("EO")
...
X = np.concatenate((dmt_epochs, closed_epochs, open_epochs), axis=0)
X = X.reshape(X.shape[0], channels, samples, kernels)
print(np.shape(X))

a = np.full((1,len(dmt_epochs)), 0)
b = np.full((1,len(closed_epochs)), 1)
c = np.full((1,len(open_epochs)), 2)
y = np_utils.to_categorical(np.concatenate((a,b,c), axis=1)[0])
print(np.shape(y))
...
# EEGNet. Both the original model and the revised model are implemented.
