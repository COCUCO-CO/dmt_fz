...
class AnimationGenerator:
            ...
            with ui.column().classes('w-full p-3 gap-3'):
                ...
                with ui.row().classes('gap-3 items-center flex-wrap'):
                    ...
                    ui.select(
                        ...
                        'update:model-value',
                        lambda e: setattr(self, '_band', extract_event_value(e.args))
                    )
                    ...
                    ui.select(
                        ...
                        'update:model-value',
                        lambda e: setattr(self, '_mode', extract_event_value(e.args))
                    )
                    ...
                    ui.select(
                        ...
                        'update:model-value',
                        lambda e: setattr(self, '_quality', extract_event_value(e.args))
                    )
                ...
                # Log/preview area (after buttons so buttons are always visible)
                self._log_container = ui.column().classes('w-full items-center')
                self._log_container.props('id="animation-log"')
                self._log_container.style(
                    'background: #050505; border-radius: 4px; padding: 12px;'
                )
    ...
    def _show_fullsize(self, path: str, media_type: str) -> None:
        """Show image or video in fullsize dialog."""
        with ui.dialog() as dialog, ui.card().style(
            'max-width: 95vw; max-height: 95vh; padding: 8px; background: #111;'
        ):
            with ui.column().classes('items-center'):
                if media_type == 'image':
                    ui.image(path).style('max-width: 90vw; max-height: 85vh; object-fit: contain;')
                else:
                    ui.video(path).style('max-width: 90vw; max-height: 85vh;')
                ui.button('Cerrar', on_click=dialog.close, icon='close').props('flat')\
    ...
    def _load_subjects(self) -> None:
        ...
        if subfolders:
            ...
            self._log(f'✓ {len(subfolders)} carpetas: {", ".join(subfolders)}', THEME_PRIMARY)
        else:
    ...
    def _load_subjects_from_folder(self, folder: Path) -> None:
        ...
        if subjects:
            ...
            self._log(f'✓ {len(subjects)} sujetos en {folder.name}/', THEME_PRIMARY)
        else:
    ...
    async def _generate_frames(self) -> None:
        ...
        subject = extract_event_value(self._subject_select.value) if self._subject_select else None
        if not subject:
        ...
        band = str(self._band) if self._band else 'Alpha'
        mode = str(self._mode) if self._mode else 'stc'
            ...
            if process.returncode == 0:
                ...
                if expected_output.exists():
                    ...
                    if frames:
                        ...
                        self._log_container.clear()
                        with self._log_container:
                            ui.label(f'✓ Generated {len(frames)} frames').style(
                                f'color: {THEME_PRIMARY}; font-family: JetBrains Mono; '
                                f'font-size: 0.75rem; margin-bottom: 8px;'
                            )
                            # Thumbnail preview
                            img = ui.image(str(first_frame)).style(
                                'max-width: 512px; max-height: 400px; object-fit: contain; '
                                'border-radius: 4px; cursor: pointer;'
                            )
                            img.on('click', lambda f=str(first_frame): self._show_fullsize(f, 'image'))
                            ui.label('Click en la imagen para ver en tamaño completo').style(
                                f'color: {THEME_TEXT_DIM}; font-size: 0.65rem; margin-top: 4px;'
                            )
                    else:
    ...
    async def _generate_video(self) -> None:
        ...
        output_video = frames_dir.parent / f'{subj}_{cond}_{band}_animation.mp4'
        ...
        try:
            ...
            if process.returncode == 0:
                if output_video.exists():
                    self._log_container.clear()
                    with self._log_container:
                        ui.label(f'✓ Video: {output_video.name}').style(
                            f'color: {THEME_PRIMARY}; font-family: JetBrains Mono; '
                            f'font-size: 0.75rem; margin-bottom: 8px;'
                        )
                        # Thumbnail video
                        ui.video(str(output_video)).style(
                            'max-width: 512px; max-height: 400px; border-radius: 4px;'
                        )
                        with ui.row().classes('gap-2 mt-2'):
                            ui.button(
                                'Ver Full Size', 
                                on_click=lambda v=str(output_video): self._show_fullsize(v, 'video'),
                                icon='fullscreen'
